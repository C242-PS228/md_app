...
class AuthRepository(
    ...
    suspend fun loginUser(request: LoginRequest) : LoginResponse {
        return try {
            ...
            if (response.status == "success" && !response.token.isNullOrEmpty()) {
                ...
                Log.d("AuthRepository", "Token saved successfully: ${response.token}")
            } else {
                Log.e("AuthRepository", "Empty token or login failed")
    ...
    suspend fun saveToken(token: String) {
        try {
            Log.d("AuthRepository", "Token saved successfully: $token")
